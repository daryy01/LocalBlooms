...
export let sellers = JSON.parse(localStorage.getItem('sellers')) || [
  { 
    id: 1, 
...
// Default products and add-ons (you can add these dynamically as well)
...
export const addShop = (shop) => {
  const newShop = {
    id: sellers.length + 1,  // Ensure each shop has a unique id
    name: shop.name,
  ...
  sellers.push(newShop);  // Add new shop to the array

  // Also add the new shop to the products and add-ons (you can customize this)
  products[newShop.id] = [];
  flowerAddOns.sellerAddOns[newShop.id] = [];

  // Save updated sellers and data to localStorage
  localStorage.setItem('sellers', JSON.stringify(sellers));
  localStorage.setItem('products', JSON.stringify(products));
  localStorage.setItem('flowerAddOns', JSON.stringify(flowerAddOns));
};

export const removeShop = (shopId) => {
  const existingShops = JSON.parse(localStorage.getItem('shops')) || [];
  const updatedShops = existingShops.filter(shop => shop.id !== shopId); // Remove shop by id
